  ...
  export default {
    ...
    methods: {
      init (id) {
        this.visible = true
        this.$http({
          url: this.$http.adornUrl('/web/oss/controller/oss/config'),
          method: 'get',
          params: this.$http.adornParams()
        }).then(({data}) => {
          this.dataForm = data && data.code === 200 ? data.result : []
        })
      },
      // 表单提交
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl('/web/oss/controller/oss/saveConfig'),
              method: 'post',
